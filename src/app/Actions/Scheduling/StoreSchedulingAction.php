...
class StoreSchedulingAction
{
    ...
    public function asController(ActionRequest $request): RedirectResponse
    {
        ...
        $scheduling = $this->handle($validated, null);
    ...
    public function handle(array $validated, ?Scheduling $scheduling = null): Scheduling
    {
        ...
        if (Scheduling::employeeHasOverlap((int) $validated['employee_id'], $startNorm, $endNorm, null)) {
            throw ValidationException::withMessages([
        ...
        try {
            ...
            if ($code === null || Scheduling::where('tenant_id', tenant('id'))->where('code', $code)->exists()) {
                $code = Scheduling::generateNextCode();
            }

            $createScheduling = function (string $useCode) use ($validated, $start, $end, $accountId) {
                return Scheduling::create([
                    ...
                    'color' => $validated['color'] ?? '#4267b2',
                    'notes' => $validated['notes'] ?? null,
                ]);
            };

            try {
                $scheduling = $createScheduling($code);
            } catch (QueryException $e) {
                if (($e->errorInfo[0] ?? '') === '23505' && str_contains($e->getMessage(), 'schedulings')) {
                    $code = Scheduling::generateNextCode();
                    $scheduling = $createScheduling($code);
                } else {
                    throw $e;
