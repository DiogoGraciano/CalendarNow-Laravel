...
class UpdateEmployeeDayOffAction
{
    ...
    public function asController(EmployeeDayOff $employeeDayOff, ActionRequest $request): RedirectResponse
    {
        ...
        $this->handle($validated, $employeeDayOff);

        return redirect()
            ->route('employee-days-off.index')
            ->with('success', 'Folga atualizada com sucesso');
    }

    /**
     * @param  array<string, mixed>  $validated
     */
    public function handle(array $validated, EmployeeDayOff $employeeDayOff): void
    {
        $employeeDayOff->update($validated);
    }
